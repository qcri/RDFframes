from src.query_buffer.query_operators.grouped.groupby_seed_operator import GroupBySeedOperator
from src.query_buffer.query_operators.grouped.grouped_aggregation_operator import GroupedAggregationOperator
from src.query_buffer.query_operators.grouped.having_operator import HavingOperator
...
from src.utils.constants import AggregationFunction
from src.utils.constants import JoinType
...
class GroupedDataset(Dataset):
    ...
    def min(self, aggregation_fn_data):
        """
        Runs min aggregate function on the pass src columns list and returns the min of the passed columns.
    ...
    def max(self, aggregation_fn_data):
        """
        Runs max aggregate function on the pass src columns list and returns the max of the passed columns.
        if the col in the src_cols_list is a non-group_by col in the parent dataset the the returned dataset will have
        a new col and corresponding value in dst_list_tags will be the new_col_name.
        if the new col in the src_cols_list is a group_by col in this dataset, return a dictionary where the
    ...
    def count(self, aggregation_fn_data):
        """
        Runs count aggregate function on the pass src columns list and returns the count of the passed columns.
