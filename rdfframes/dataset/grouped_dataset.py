...
class GroupedDataset(Dataset):
    ...
    def join(self, dataset2, join_col_name1, join_col_name2=None, new_column_name=None, join_type=JoinType.InnerJoin):
        ...
        if join_col_name1 not in self.grouping_cols:
            raise Exception(
                "Join key specified for the first dataset is not one of groupby columns")
        if  dataset2.type() == "GroupedDataset":
            if join_col_name2 not in dataset2.grouping_cols:
                raise Exception(
                    "Join key specified for second dataset is not one of groupby columns")
